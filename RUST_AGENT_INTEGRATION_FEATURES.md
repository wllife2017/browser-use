...
245. Rust Agent browser_script observe-threshold speed parity
   - Terminal browser_script startup now waits 7 seconds before returning a detachable observe run instead of 15 seconds, keeping enough initial wait for slow page/script setup while reducing repeated per-step latency in eval tasks.
   - Local eval service and GitHub eval workflow both pin `BU_BROWSER_SCRIPT_INITIAL_WAIT_MS=7000` so local gates and runner-dispatched gates use the same Rust-core behavior.
...
276. Rust Agent Laminar and cache-write usage clarity
   - Rust Laminar LLM replay now passes the model-visible message array directly as the `LLM` span input and the assistant message array as span output, avoiding the extra `{model, provider, messages}` wrapper that made Laminar render nested input strangely.
   - Terminal request observability now emits the full model-visible request without the old message window or text budget, and keeps inline media data in `llm_input`; the Python replay converts Rust `media` parts into OpenAI-style `image_url` data URLs so Laminar can render screenshots inline while secret-like fields remain redacted.
   - Model, provider, tool inventory, previews, and usage details now move to scalar span attributes, including indexed `gen_ai.prompt.N.*` / `gen_ai.completion.N.*` attributes expected by Laminar/OpenTelemetry LLM spans.
   - Rust usage summaries now expose `total_prompt_cache_creation_tokens` and `total_prompt_cache_creation_cost`, and token-count reconstruction sums per-call cache-write tokens even when cumulative `total_token_usage` omits the named cache-write field while keeping it in `total_tokens`.
   - Direct Python-interface diagnostic through `from browser_use.rust import Agent` with Browser Use cloud completed a 3-step `https://example.com` task successfully and proved the cost breakdown: `$0.11142345` total, including `$0.09388125` for 25,035 cache-write tokens and `$0.0110772` for 36,924 cache-read tokens.
   - Proof: terminal `cargo test -p browser-use-agent turn_request_event_carries_full_llm_input_messages -- --nocapture`, terminal `cargo test -p browser-use-agent turn_request_event_carries_all_observability_messages_without_text_budget -- --nocapture`, terminal `cargo fmt --check`, browser-use `.venv/bin/python -m pytest -q tests/ci/test_rust_agent.py -k 'rust_terminal_usage_prices_token_count_events or rust_terminal_usage_prices_anthropic_raw_cache_reads or rust_terminal_usage_sums_token_count_cache_creation or rust_token_summary_does_not_double_count_cache_reads or rust_agent_laminar_run_summary_populates_current_span'`, browser-use `.venv/bin/python -m py_compile browser_use/rust/service.py browser_use/tokens/service.py browser_use/tokens/views.py`, and the saved diagnostic conversation at `/tmp/rust_usage_diag_conv`.

277. Rust Agent exact Laminar tool replay
   - Python `browser_use.rust.Agent` Laminar replay now creates child `TOOL` spans for terminal tool calls, including tool arguments, status/error attributes, structured text results, and inline image parts when the terminal event carries them.
   - Rust LLM spans now include the full model-visible tool catalog in the Laminar span input, Laminar's current `gen_ai.tool.definitions` field, and backwards-compatible `llm.request.functions.*` attributes, including browser_script descriptions and input schemas.
   - Rust LLM spans now emit full `gen_ai.input.messages` / `gen_ai.output.messages` arrays in addition to indexed legacy prompt/completion attributes, so Laminar can render the same system text, user text, images, tool calls, and assistant outputs the model saw without our old wrapper nesting.
   - Rust LLM spans now include cache-read/cache-write usage and per-bucket cost attributes derived from the same official usage fields used by Browser Use token accounting, so Laminar and the eval dashboard can be compared without hidden pricing heuristics.
   - Timeout/error reconstruction now preserves the latest streamed assistant text as the visible final result when the terminal process fails after an LLM turn has already produced content, while keeping the original error on the history.
   - Proof: browser-use `uv run pytest -q tests/ci/test_rust_agent.py -k 'laminar_run_summary or loads_started_session_events_after_terminal_timeout' --disable-warnings`, browser-use `uv run python -m py_compile browser_use/rust/service.py tests/ci/test_rust_agent.py`, and browser-use `git diff --check -- browser_use/rust/service.py tests/ci/test_rust_agent.py`.

278. Rust terminal structured replay and cache-window parity
   - Terminal session reconstruction now falls back to structured `summary`, `data`, and `outputs` fields when browser_script and other tools return successful structured payloads with empty `text`/`content`, preventing future LLM turns and Laminar replay from seeing a blank tool result while the agent actually received evidence.
   - The Rust request builder keeps the latest model-visible message cacheable and adds one earlier non-system cache breakpoint for long histories, staying within Anthropic's four-breakpoint limit together with the system prompt and tool schema breakpoint.
   - Terminal `model.turn.request` events now carry the full sanitized `llm_input.tools` array and repeated model round-trips from the same driver emit monotonic `turn_idx` values, so Laminar can display tool descriptions and order turns without relying only on span order.
   - This is a general cache-window fix for long browser/research trajectories; it does not add task-specific instructions or domain-specific behavior.
   - Proof: terminal `cargo test -q -p browser-use-agent repeated_sampling_requests_emit_monotonic_turn_indices -- --nocapture`, terminal `cargo test -q -p browser-use-agent fused_driver_advertises_dispatcher_tool_specs_on_request -- --nocapture`, terminal `cargo test -q -p browser-use-agent tool_output_event_uses_structured_browser_script_fallback_text -- --nocapture`, terminal `cargo test -q -p browser-use-agent open_stream_marks_an_earlier_cache_breakpoint_for_long_histories -- --nocapture`, terminal `cargo test -q -p browser-use-agent turn_request_event_carries_full_llm_input_messages -- --nocapture`, terminal `cargo test -q -p browser-use-agent turn_request_event_carries_all_observability_messages_without_text_budget -- --nocapture`, terminal `cargo fmt --check`, terminal `git diff --check -- crates/browser-use-agent/src/session/reconstruct.rs crates/browser-use-agent/src/session/reconstruct_tests.rs crates/browser-use-agent/src/turn/sampling.rs crates/browser-use-agent/src/turn/sampling_tests.rs`, and terminal `cargo build -q -p browser-use-cli --bin browser-use-terminal`.

279. Rust Agent eval timeout/error finalization hardening
   - Python `browser_use.rust.Agent` history reconstruction now treats an empty or non-text latest assistant stream as absent text instead of raising `AttributeError: 'NoneType' object has no attribute 'strip'` while rebuilding a failed terminal run.
   - The eval runner now derives dashboard `tokensUsed` from `history.usage.total_tokens` when Rust histories lack per-step Python metadata, while still sending the full structured `usage` object with cache and cost buckets.
   - Five-task real_v8 local-platform gate `kh703qjpt46yfdkyv741pecgq58809h6` used local browser-use `48b15aaf`, terminal `9102fb2`, `BU_RUNTIME=brust`, Browser Use cloud CDP with US proxy, `--parallel-runs 5`, `--max-steps 100`, `--task-timebox-seconds 600`, Claude Sonnet 4.6 executor, and Agent SDK judge.
   - Gate outcome: all five Browser Use cloud sessions created promptly and all five tasks saved to Convex. Scores were task 1 `0.50`, task 2 `0.85`, task 3 `0.90`, task 4 failed from the now-fixed reconstruction bug, and task 5 `0.40`; pipeline success was 4/5, saved-score mean was `0.6625`, missing-task-as-zero mean was `0.53`.
   - Observed general risks: long trajectories were the main cost/speed issue (tasks 1 and 5 ran 428s/436s and cost `$7.33`/`$3.49`), Laminar export hit resource/deadline limits on very large traces, and one AUD vendor task reached a Cloudflare captcha despite Browser Use cloud proxy. These are not yet fixed because this gate showed them as evidence to investigate across more tasks rather than single-domain overfitting.
   - Proof: browser-use `uv run pytest -q tests/ci/test_rust_agent.py -k 'process_failure_ignores_empty_stream_text or loads_started_session_events_after_terminal_timeout or laminar_run_summary' --disable-warnings`, browser-use `uv run python -m py_compile browser_use/rust/service.py tests/ci/test_rust_agent.py`, evaluations-internal `PYTHONPATH=. uv run pytest -q tests/test_service_cli.py -k 'uses_usage_total_tokens_when_metadata_missing or synthesizes_partial_result_on_timeout or pre_judge_checkpoint'`, evaluations-internal `PYTHONPATH=. uv run python -m py_compile eval/service.py tests/test_service_cli.py`, and dashboard query `/api/getRunResults?runId=kh703qjpt46yfdkyv741pecgq58809h6&format=only_judge`.

280. Rust eval oversized dashboard history persistence
   - The eval runner now trims only Convex-facing `actionHistory` and `completeHistory` text fields before save, keeping large extracted-content runs below the Convex 1 MiB field limit while preserving local `saved_trajectories` files, screenshots, final result text, structured usage/cost data, and Laminar traces.
   - This addresses a general runner failure from real_v8 task 6 where a single `Captured requests` extraction was about 2.29 MiB and was duplicated into both dashboard history fields. Before the fix, the agent and judge ran but final `saveTaskResult` failed with `Value is too large`, making the task appear missing/empty.
   - Five-task real_v8 local-platform gate `kh71j6cq0cwegjjb6rh4c4840h881wm8` used local browser-use `6c7e4e38`, terminal `9102fb2`, `BU_RUNTIME=brust`, Browser Use cloud CDP with US proxy, `--parallel-runs 5`, `--max-steps 100`, `--task-timebox-seconds 600`, Claude Sonnet 4.6 executor, and Agent SDK judge for tasks 6-10.
   - Gate outcome before the persistence fix was 4/5 saved results: task 7 `1.0`, task 8 `0.8`, task 9 `0.85`, task 10 `0.5`; task 6 reached judge score `0.0` but failed final dashboard save on the oversized payload. The recorded four-task mean was `0.7875`; treating task 6 as zero gives `0.63`.
...
289. Rust Agent wrapper initialization and public eval ref parity
   - `browser_use.rust.Agent` now owns its Browser Use signature-alignment hook instead of importing the Rust wrapper from the Python `Agent` module at import time, preserving `from browser_use.rust import Agent` while keeping the previous Python `Agent` class available as `_PythonAgent`.
...
- 2026-06-04 post-dataset-timebox five-task real_v8 Agent SDK gate:
  ...
  - Saved results before the 10-minute cap: task 9 score `0.5` in 252.21s agent time/302.09s total pipeline and task 10 score `1.0` in 549.82s/586.58s. Result IDs were `kn7292v9fqnspg0pkjs7rr2355881gpm` and `kn74ct6r2trssf3sbh15fa34hh880smb`.
  - Tasks 6, 7, and 8 were still in agent-stage execution when the cap fired. Their remaining Browser Use cloud sessions were explicitly stopped after the cap, and no eval/terminal subprocesses remained.
  - The prior Agent SDK `success`/max-turn exception did not recur for completed tasks: both saved tasks logged `Agent SDK judge trajectory-only mode enabled; skipping tool-enabled verification attempt` and saved normally. One Laminar Claude proxy health-check restart and Convex progress-save read timeouts appeared but did not break judging.
  - This gate does not justify scaling to 50: saved-task mean score was `0.75`, but missing-task-as-zero mean score was `0.30` with only 2/5 tasks saved within the iteration budget. Prompt-only timebox guidance did not force long research/document tasks to finalize before the external cap.
- 2026-06-04 post-text-heavy-screenshot-restraint five-task real_v8 Agent SDK gate:
  - Run `kh72amba59xvvhw18x2pgj3vfd880xp5` used `BU_RUNTIME=brust`, eval-internal commit `6e75e49`, terminal commit `3453591`, browser-use commit `ae62bea2`, `--browser browser-use-cloud`, `BROWSER_USE_CLOUD_PROXY_COUNTRY_CODE=us`, `BU_BROWSER_SCRIPT_INITIAL_WAIT_MS=7000`, `AGENT_SDK_JUDGE_TIMEOUT_SECONDS=25`, `AGENT_SDK_JUDGE_FALLBACK_ATTEMPTS=3`, `AGENT_SDK_JUDGE_CONCURRENCY=16`, `LLM_BROWSER_CAPTURE_FPS=0`, `BU_DISABLE_FALLBACK_CAPTURE_GIF=1`, `BU_BROWSER_SCRIPT_SESSION_OUTPUTS=1`, `--use-vision false`, `--judge-type agent_sdk`, `--model claude-sonnet-4-6`, `--eval-model claude-sonnet-4-6`, `--parallel-runs 5`, and `--max-steps 100` for real_v8 tasks 6-10.
  - Browser Use cloud startup was healthy: all five sessions were created around 2026-06-04T12:22:23Z, all five tasks entered Rust-core agent execution, and no CAPTCHA/local-browser misconfiguration appeared.
  - Saved results before the 10-minute cap: task 9 score `1.0` in 202.27s agent time/228.65s total pipeline and task 10 score `1.0` in 410.01s/450.59s. Result IDs were `kn77qash6gyqzagq6ts0qawdpd8818cq` and `kn76ghc82n0agbjtcv325ccc8x880hvj`.
  - Tasks 6, 7, and 8 were still in agent-stage execution when the cap fired. Their remaining Browser Use cloud sessions were explicitly stopped after the cap, and no eval/terminal subprocesses remained.
  - The prior Agent SDK `success`/max-turn exception did not recur for completed tasks: both saved tasks logged `Agent SDK judge trajectory-only mode enabled; skipping tool-enabled verification attempt` and saved normally.
  - This gate does not justify scaling to 50: saved-task mean score was `1.0`, but missing-task-as-zero mean score was `0.40` with only 2/5 tasks saved within the iteration budget. Text-heavy screenshot restraint reduced some non-vision artifact volume, but long-tail agent execution still dominated tasks 6-8.
...
- 2026-06-04 post-batch-fetch-helper five-task real_v8 Agent SDK gate:
  ...
  - Saved results before the 10-minute cap: task 9 score `0.8` in 266.62s agent time/339.23s total pipeline, task 10 score `0.85` in 411.98s/489.87s, and task 6 score `1.0` in 423.63s/490.24s.
  - Task 9 showed a concrete cost/step improvement over the previous summary-first gate: 17 steps and `725178` total tokens instead of 22 steps and `1267194` total tokens. Task 6 also completed and saved before the cap after being unsaved in the previous gate.
  - Current fresh artifacts did not show direct calls to `http_get_many`, `browser_fetch`, or `browser_fetch_many`, so this gate should not be treated as proof that the new helper API has been adopted by the model in live evals yet.
  - Tasks 7 and 8 were still in agent-stage execution when the cap fired. All five Browser Use cloud sessions were explicitly stopped after the cap, and no eval/terminal subprocesses remained.
  - This gate does not justify scaling to 50: saved-task mean score was about `0.88`, but missing-task-as-zero mean score was `0.53` with only 3/5 tasks saved within the iteration budget. The remaining bottleneck is still long-tail document/research execution and cumulative prompt growth.
- 2026-06-04 post-summary-first-browser-script five-task real_v8 Agent SDK gate:
  - Run `kh77e9y1cjeyygdnvex85qwcrs880hr0` used `BU_RUNTIME=brust`, terminal commit `297930a`, `--browser browser-use-cloud`, `BROWSER_USE_CLOUD_PROXY_COUNTRY_CODE=us`, `BU_BROWSER_SCRIPT_INITIAL_WAIT_MS=7000`, `AGENT_SDK_JUDGE_TIMEOUT_SECONDS=25`, `AGENT_SDK_JUDGE_FALLBACK_ATTEMPTS=3`, fallback timeout default floor `45`, `AGENT_SDK_JUDGE_CONCURRENCY=16`, `LLM_BROWSER_CAPTURE_FPS=0`, `BU_DISABLE_FALLBACK_CAPTURE_GIF=1`, `BU_BROWSER_SCRIPT_SESSION_OUTPUTS=1`, `--use-vision false`, `--judge-type agent_sdk`, `--model claude-sonnet-4-6`, `--eval-model claude-sonnet-4-6`, `--parallel-runs 5`, and `--max-steps 100` for real_v8 tasks 6-10.
  - Browser Use cloud startup was healthy: all five sessions were created around 2026-06-04T10:54:10Z, all five tasks entered Rust-core agent execution, and no CAPTCHA/local-browser misconfiguration appeared.
  - Saved results before the 10-minute cap: task 10 score `1.0` in 229.10s agent time/289.97s total pipeline, task 9 score `0.75` in 312.69s/391.38s, and task 8 score `0.8` in 456.07s/532.48s.
  - The prior fatal Agent SDK `success` exception did not recur. Saved tasks used the bounded judge fallback after 25-second tool-judge timeouts and saved scores normally.
  - Compared with the previous 4 KiB gate, task 10 improved materially in agent time (`383.78s` to `229.10s`) while task 9 regressed (`222.07s` to `312.69s`) and task 8 completed/saved instead of remaining unsaved.
  - Tasks 6 and 7 were still in agent-stage execution when the cap fired. All five Browser Use cloud sessions were explicitly stopped after the cap, and no eval/terminal subprocesses remained.
  - This gate does not justify scaling to 50: saved-task mean score was `0.85`, but missing-task-as-zero mean score was `0.51` with only 3/5 tasks saved within the iteration budget. The remaining bottleneck is still long-tail document/research execution and cumulative prompt growth, not cloud browser startup or judge exception handling.
...
- 2026-06-04 focused real_v8 task-4 Agent SDK diagnostic:
  - Run `kh760nrdkqwsjxb539c6v740m58808s1` used `BU_RUNTIME=brust`, `--browser browser-use-cloud`, `--judge-type agent_sdk`, `--model claude-sonnet-4-6`, `--eval-model claude-sonnet-4-6`, `--parallel-runs 1`, and `--max-steps 75`.
  - Score was `0.0` after the terminal child was stopped at 271.88s and 21 steps; the run showed vendor-domain scouting without committing to one AUD source, which feature 242 addresses.
- 2026-06-04 hardened/lowercase-proxy five-task real_v8 Agent SDK gate:
  - Run `kh7akj2ps8vg3tcfbg9g2zwj1188114a` used `BU_RUNTIME=brust`, `--browser browser-use-cloud`, `BROWSER_USE_CLOUD_PROXY_COUNTRY_CODE=us`, `--judge-type agent_sdk`, `--model claude-sonnet-4-6`, `--eval-model claude-sonnet-4-6`, `--parallel-runs 5`, `--max-steps 100`, and `AGENT_SDK_JUDGE_ATTEMPTS=3`.
...
- 2026-06-04 post-judge-fallback five-task real_v8 Agent SDK gate:
  - Run `kh74xjt5jwnhbj5kz11k7xc29s881784` used `BU_RUNTIME=brust`, `--browser browser-use-cloud`, `BROWSER_USE_CLOUD_PROXY_COUNTRY_CODE=us`, `BU_BROWSER_SCRIPT_INITIAL_WAIT_MS=7000`, `--judge-type agent_sdk`, `--model claude-sonnet-4-6`, `--eval-model claude-sonnet-4-6`, `--parallel-runs 5`, and `--max-steps 100`.
  - All five Browser Use cloud sessions were created promptly and all five tasks entered Rust-core agent execution. Saved results before the 10-minute stop: task 1 score `0.0` in 89.98s agent time/125.52s total pipeline, task 2 score `0.0` in 161.80s/213.03s, and task 3 score `1.0` in 282.02s/343.87s.
  - Tasks 4 and 5 were still running when the 10-minute limit was reached; their remaining Browser Use cloud sessions were manually stopped. No Agent SDK judge exception or max-turn fallback was observed in the saved results.
  - This gate still did not justify scaling to 50 because the sample quality was below target and the long-tail tasks did not complete within the 10-minute iteration budget.
