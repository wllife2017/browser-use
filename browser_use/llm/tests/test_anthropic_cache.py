...
class TestAnthropicCache:
	...
	def test_cache_only_last_block_in_message(self):
		...
		cache_count = sum(1 for block in content_blocks if block.get('cache_control') is not None)
		assert cache_count == 1, f'Expected 1 cache_control block, got {cache_count}'

		# Verify it's the last block
		assert content_blocks[-1].get('cache_control') is not None
		assert content_blocks[0].get('cache_control') is None
	...
	def test_cache_assistant_with_content_and_tools(self):
		...
		cache_count = sum(1 for block in content_blocks if block.get('cache_control') is not None)
		assert cache_count == 1, f'Expected 1 cache_control block, got {cache_count}'
		assert content_blocks[-1].get('cache_control') is not None  # Last tool_use block
		assert content_blocks[0].get('cache_control') is None  # First text block
		assert content_blocks[1].get('cache_control') is None  # Second text block
