...
def construct_self_verify_messages(
	task: str,
	final_result: str,
	...
	"""
	Construct lightweight verification messages to check if the agent's claimed success is justified.

	Much lighter than the full judge — just task + final result + optional last screenshot.
	"""
...
Your job is simple: given the task and the agent's final response, determine if the response genuinely satisfies ALL requirements of the task.

Check for these common failure patterns:
1. **Incorrect data**: Wrong number of items, missing filters/criteria, wrong format
2. **Unverified actions**: Agent claims to have submitted a form, posted a comment, or saved a file but there's no evidence it happened
...
Respond with EXACTLY this JSON structure:
{
	...
	"reason": "Brief explanation if not verified, empty string if verified"
}
...
Does the agent's response fully satisfy all requirements of the task? Respond with the JSON structure."""
	...
	# Add last screenshot if available and vision is enabled
	if use_vision is not False and last_screenshot_path:
		encoded = _encode_image(last_screenshot_path)
		if encoded:
