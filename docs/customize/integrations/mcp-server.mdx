...
description: "Connect AI assistants to browser-use via Model Context Protocol - cloud or self-hosted"
icon: "server"
mode: "wide"
---
...
Browser-use provides two ways to expose browser automation capabilities to AI assistants via the Model Context Protocol (MCP):

1. **Cloud MCP Server** (recommended) - Hosted service with simple setup and managed infrastructure
2. **Local Self-Hosted MCP** - Free, open-source option running on your machine

Both enable AI assistants like Claude Desktop, Claude Code, and other MCP-compatible clients to perform web automation tasks.

## Cloud MCP Server (Recommended)

Browser Use provides a hosted **Model Context Protocol (MCP)** server that enables AI assistants to control browser automation. Works with any HTTP-based MCP client.

**MCP Server URL:** `https://api.browser-use.com/mcp`

This is an **HTTP-based MCP server** designed for cloud integrations and remote access with managed infrastructure, persistent authentication profiles, and simplified setup.
...
The cloud MCP server provides three high-level tools:
...
- **use_profile** (optional): Use a cloud profile for persistent authentication (default: false)
- **profile_id** (optional): UUID of the profile to use (required when use_profile=true)
...
- **session_id** (required): UUID of the session to monitor (returned by browser_task)
